...
#'                                is provided.
#' @details
#' Note: The output of this function may be used to create plots or tables. This function relies on
#' data available in Cohort Diagnostics results data model. The function will use one of the two
#' methods to connect to the results data model, 1) database mode, and 2) in-memory mode.
#' Database mode: In this mode, R will look for the results data model in a remote relational dbms.
#' The database system should be one of the databases supported by DatabaseConnector package. If
...
#' parameters have a NULL argument then query will be in-memory mode. R will now expect and check for
#' a data frame object available in R's memory, as required for the function. The object should be an
#' object specificed in Cohort Diagnostics results data model.
#' Objects in R's memory are expected to follow camelCase naming conventions (see HADES), while
#' objects in a relational database system are expected to follow snake-case naming conventions.
