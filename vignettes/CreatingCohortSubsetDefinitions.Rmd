...
We will begin by creating a dummy OMOP data model and populating the dummy OMOP data model with subjects and drug exposure. The drug exposure itself will be recognized as ibuprofen exposure. The sample data itself is found in SaveVignetteCohortToDuckDB.R found in inst/testdata folder if interested in analyzing it.
...
After extracting those cohorts from the csv file(celcoxib and ibuprofen), we can finally reference the ibuprofen subset to celcoxib by setting the target id to the cohort id of celcoxib. That directly translates to ibuprofen subsetting off of celcoxib. 
