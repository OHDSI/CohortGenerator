...
Subset definitions are named sets of one or more operators (from above) that can be applied to a set of one or more cohorts. Operatiors can be sequentially chained within subset definitions and all outputs are full cohorts that can be passed into to other analytical tasks.
...
cohort_data <- cohorts |>
  ...
  dplyr::mutate(gender = ifelse(gender_concept_id == 8532, "Female", "Male"), 
...
ggplot(cohort_data, aes(x = cohort_start_date, xend = cohort_end_date, 
  ...
  labs(title = "Cohort subset membership by subject",
       x = "Date",
