...
# Subset Operators & Subset definitions

**Subset Operator**: A subset operator is a single, specific operation—such as filtering based on demographics or limiting events to a certain time window—that is applied to a cohort.
It represents an individual filtering step, like "include only patients exposed to ibuprofen during the study period."

**Subset Definition**: Subset definition as stated before, is a named SET of operations that can be applied to a set of one or more cohorts.
It encapsulates the overall logic for creating a derived cohort and is designed for reuse and clarity when performing broader analytic tasks.
...
For this specific test, we want to create a subset that has the logic:
...
Down below, we are extracting sampled cohorts that mimic what the actual cohort would be like, produced by a script called PackageData.R found in extras.
