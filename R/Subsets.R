...
# VisitSubsetOperator ------------------------------
#' @title Visit Subset Operator
#' @export
#' @description
#' A subset of type visit - subset a population to only those with a specic visit type
VisitSubsetOperator <- R6::R6Class(
  classname = "VisitSubsetOperator",
  inherit = SubsetOperator,
  private = list(
    queryBuilder = VisitSubsetQb,
    suffixStr = "Visit",    
    .visitConceptIds = c()
  ),
  public = list (
    ...
    toList = function() {
      objRepr <- super$toList()
      if (length(private$.visitConceptIds)) {
        objRepr$visitConceptIds <- private$.visitConceptIds
      }
      objRepr
    },
    #' Get auto generated name
  ...
  active = list (
    visitConceptIds = function(visitConceptIds) {
      if (missing(visitConceptIds)) {
        return(private$.visitConceptIds)
      }
      checkmate::assertList(as.list(visitConceptIds), types="numeric", min.len = 0)
      private$.visitConceptIds <- visitConceptIds
      return(self)
    }
  )
)
...
createErVisitSubset <- function(name) {
  createVisitSubset(name = name, visitConceptIds = 9203)
}

createOutpatientVisitSubset <- function(name) {
  createVisitSubset(name = name, visitConceptIds = 9202)
}