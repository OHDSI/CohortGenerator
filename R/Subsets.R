...
SubsetOperator <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Not intended to be used - should be imnplemented in subclasses
...
CohortSubsetOperator <- R6::R6Class(
  ...
  public = list(
    ...
    #' @return character
    getAutoGeneratedName = function() {
      nameString <- paste0(private$suffixStr, ": {")
      if (self$negate) {
        nameString <- paste0(nameString, "subjects not in ")
      } else {
        nameString <- paste0(nameString, "subjects in ")
      }
...
DemographicSubsetOperator <- R6::R6Class(
  ...
  public = list(
    ...
    #' @return character
    getAutoGeneratedName = function() {
      startStr <- paste0(private$suffixStr, ": {")
      nameString <- startStr
      if (length(self$ageMin)) {
        nameString <- paste0(nameString, "age >= ", self$ageMin)
      }
      if (length(self$ageMax)) {
        if(length(self$ageMin)) {
           nameString <- paste0(nameString, " and ")
        }
        nameString <- paste0(nameString, "age <= ", self$ageMax)
      }
      if (!is.null(self$gender)) {
        if (nameString != startStr)
          nameString <- paste0(nameString, " and ")

        nameString <- paste0(nameString, "Gender concept ",  paste(self$gender, collapse = ", "))
      }
      if (!is.null(private$.race)) {
        if (nameString != startStr)
          nameString <- paste0(nameString, " and ")

        nameString <- paste0(nameString, "Race concept ", paste(self$race, collapse = ", "))
      }
      if (!is.null(private$.ethnicity)) {
        if (nameString != startStr)
          nameString <- paste0(nameString, " and ")

        nameString <- paste0(nameString, "Ethnicity concept ", paste(self$ethnicity, collapse = ", "))
      }
